export const Reference = ({
  ...
  excerpt,
        ...
        <fabric-reference>
          ...
          <span slot="metadata-text-one">${text}</span>
          <span slot="metadata-text-two">${text2}</span>
...
export const ReferenceGroupEl = () => {
  const references = [];
  for (var i = 0; i < 5; i++) {
    references.push(
      Reference({
        index: i,
        label: `Fabric Reference ${i}`,
        text: "Text One",
        text2: "Text Two",
        excerpt: `Fabric Reference ${i} Originates at the...`,
      })
    );
  }
